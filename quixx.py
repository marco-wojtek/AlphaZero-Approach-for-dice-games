...
point_space = np.array([0,1,3,6,10,15,21,28,36,45,55,66,78])
...
def get_points_and_terminated(self,state):
    ...
    for n in range(num_players):
        for m in range(4):
            x = state[n+1][m][-1] == 1
            points[n] += point_space[np.count_nonzero(state[n+1][m][np.where(state[n+1][m]==1)])+x]
...
def greedy_bot(state,valid_moves,player):
    last_marked = np.array([])
    for i in range(4):
        x = np.argwhere(state[player+1][i]==1)
        x = action_space[i][x[-1]] if len(x)>0 else action_space[i][0]
...
for i in tqdm(range(10000)):
    game = get_initial_state(0,4)
    num_players = len(game)-2
    points, terminated = get_points_and_terminated(0,game)
    player = 0
    while not terminated:
        game[0] = dice()
        #white_dice
        act = greedy_bot(game,get_valid_moves(0,game,player,True),player)
        game = get_next_state(0,game,player,act)
        play = (player+1)%num_players
        while play != player:
            act_r = greedy_bot(game,get_valid_moves(0,game,play,True),player)
            game = get_next_state(0,game,play,act_r)
            play = (play+1)%num_players
        #rest of turn
        act_2 = greedy_bot(game,get_valid_moves(0,game,player),player)
        #if the sum is >0 at least one combination was used
        if act + act_2 == 0:
            act_2 = -1
        game = get_next_state(0,game,player,act_2)
        player = (player+1)%num_players
        points, terminated = get_points_and_terminated(0,game)
        # if act not in dict:
        #     dict[act] = 0
        # dict[act] += 1
        # if act_2 not in dict:
        #     dict[act_2] = 0
        # dict[act_2] += 1
