...
class AlphaZeroParallel:
    ...
    def learn(self):
        for iteration in range(self.args['num_iterations']):
            ...
            torch.save(self.model.state_dict(), f"NewVersionCoded/ModelsNN3/version_{loss_idx}_model_{iteration}.pt")
            torch.save(self.optimizer.state_dict(), f"NewVersionCoded/ModelsNN3/version_{loss_idx}_optimizer_{iteration}.pt")
...
if save_losses:
    open(f'NewVersionCoded/ModelsNN3/Losses{loss_idx}/policy_loss.txt', 'w').close()
    open(f'NewVersionCoded/ModelsNN3/Losses{loss_idx}/value_loss.txt', 'w').close()
    open(f'NewVersionCoded/ModelsNN3/Losses{loss_idx}/total_loss.txt', 'w').close()
#
testParallel()
